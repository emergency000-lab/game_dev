...
const chapters = [
  { name: 'Village', target: 1200, speed: 9, sky: '#7bdcff', fact: 'Rath Yatra means “chariot journey”. It celebrates Lord Jagannath, Balabhadra, and Subhadra travelling in grand wooden chariots.' },
  { name: 'Town', target: 1650, speed: 10.2, sky: '#8ad8ff', fact: 'Lord Jagannath is revered with Lord Balabhadra and Devi Subhadra. The Rath carries their presence with dignity and devotion.' },
  { name: 'Temple Street', target: 2150, speed: 11.5, sky: '#ffc777', fact: 'Balabhadra is traditionally associated with strength, protection, and elder-brother care within the sacred trio.' },
  { name: 'Grand Puri Procession', target: 2700, speed: 13, sky: '#ffad5c', fact: 'Subhadra is worshipped with deep affection and respect. In this game, all deity visuals remain calm, reverent, and idle-only.' },
  { name: 'Bada Danda', target: 3250, speed: 14.2, sky: '#f8a94f', fact: 'Bada Danda is the grand avenue of Puri where the chariots move amid devotion, music, flowers, and community service.' },
  { name: 'Gundicha Temple Route', target: 3800, speed: 15.4, sky: '#586db2', fact: 'Hera Panchami, Snana Purnima, and Niladri Bije are important festival moments connected with Rath Yatra traditions.' },
  { name: 'Festival Finale', target: 4500, speed: 16.6, sky: '#222a55', fact: 'The three famous chariots are Nandighosha for Jagannath, Taladhwaja for Balabhadra, and Darpadalana for Subhadra.' },
];
...
const obstacleTypes = ['broken road', 'buffalo crossing', 'cow crossing', 'street vendor', 'festival barricade', 'electric pole', 'rain puddle', 'loose stones', 'festival cart', 'road construction', 'moving vehicle', 'sharp turn', 'crowd congestion'];
const powerUps = ['Jagannath Blessing', 'Sudarshan Chakra Shield', 'Mahaprasad Health', 'Golden Rope', 'Temple Bell Slow Motion', 'Divine Light 2X'];
const eventTypes = ['Sudden Rain', 'Heavy Crowd', 'Flower Rain', 'Temple Bells', 'Sunrise', 'Golden Evening', 'Night Procession', 'Lightning', 'Windstorm'];
...
const state = {
  ...
  obstacles: [], devotees: [], collectibles: [], particles: [], power: null, powerLeft: 0, combo: 1, boostLeft: 0, brakeLeft: 0, divineReady: 100, bellReady: 100, event: 'Sunrise', eventLeft: 600,
};
...
function resetGame() {
  Object.assign(state, { running: true, paused: false, time: 0, shake: 0, chapter: 0, levelScore: 0, score: 0, coins: save.coins || 0, blessing: 0, health: 100, happiness: 100, speed: chapters[0].speed, spawnTimer: 35, devoteeTimer: 40, collectibleTimer: 60, powerTimer: 500, obstacles: [], devotees: [], collectibles: [], particles: [], power: null, powerLeft: 0, combo: 1, boostLeft: 0, brakeLeft: 0, divineReady: 100, bellReady: 100, event: 'Sunrise', eventLeft: 600 });
...
function updateHud() { scoreEl.textContent = Math.floor(state.score).toLocaleString(); coinsEl.textContent = state.coins.toLocaleString(); blessingEl.textContent = `${Math.floor(state.blessing)}%`; healthEl.textContent = `${Math.max(0, Math.floor(state.health))}%`; powerEl.textContent = state.power || (state.divineReady >= 100 ? 'Ready' : `${Math.floor(state.divineReady)}%`); }
function move(dir) { if (state.running && !state.paused) rath.lane = Math.max(0, Math.min(2, rath.lane + dir)); }
function speedBoost() { if (!state.running || state.paused) return; state.boostLeft = 95; navigator.vibrate?.(18); }
function brakeSlide() { if (!state.running || state.paused) return; state.brakeLeft = 105; addParticles(project(rath.x, 0).x, H - 145, '#c56d39', 12); }
function activateDivinePower() { if (!state.running || state.paused || state.divineReady < 100) return; state.power = 'Jagannath Blessing'; state.powerLeft = 260; state.divineReady = 0; state.blessing = Math.min(100, state.blessing + 8); navigator.vibrate?.([20, 30, 20]); }
function activateTempleBell() { if (!state.running || state.paused || state.bellReady < 100) return; state.power = 'Temple Bell Slow Motion'; state.powerLeft = 210; state.bellReady = 0; addParticles(W / 2, H / 2, '#ffd54a', 28); navigator.vibrate?.(45); }
...
function drawBackground() {
  ...
  const g = ctx.createLinearGradient(0, 0, 0, H); g.addColorStop(0, state.event === 'Night Procession' ? '#182143' : chapter.sky); g.addColorStop(.55, state.event === 'Sudden Rain' ? '#9db5c8' : '#fff0b2'); g.addColorStop(1, '#d98b55'); ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
  if (state.event === 'Sudden Rain') { ctx.strokeStyle = 'rgba(210,235,255,.55)'; ctx.lineWidth = 3; for (let i = 0; i < 70; i++) { const x = (i * 47 + state.time * 9) % W; const y = (i * 83 + state.time * 16) % H; ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x - 18, y + 34); ctx.stroke(); } }
  if (state.event === 'Flower Rain') { for (let i = 0; i < 55; i++) { ctx.fillStyle = i % 2 ? '#ff4f9a' : '#ffd54a'; ctx.fillRect((i * 61 + state.time * 3) % W, (i * 97 + state.time * 5) % H, 8, 8); } }
  ctx.fillStyle = 'rgba(255,248,226,.86)'; ctx.font = '800 24px system-ui'; ctx.fillText(state.event, 28, 76);
...
function drawRath() { rath.x += (lanes[rath.lane] - rath.x) * .24; const p = project(rath.x, 0); const bob = Math.sin(state.time * .18) * 4; const wheelSpin = state.time * (state.boostLeft ? .32 : .2); ctx.save(); ctx.translate(p.x + (Math.random()-.5)*state.shake, p.y + bob); ctx.scale(1.05,1.05); ctx.fillStyle='rgba(54,24,9,.25)'; ctx.beginPath(); ctx.ellipse(0,78,132,26,0,0,7); ctx.fill(); ctx.strokeStyle='rgba(105,58,28,.78)'; ctx.lineWidth=8; ctx.beginPath(); ctx.moveTo(-124,-42+Math.sin(state.time*.15)*5); ctx.quadraticCurveTo(-190,-96,-250,-120); ctx.moveTo(124,-42+Math.cos(state.time*.15)*5); ctx.quadraticCurveTo(190,-96,250,-120); ctx.stroke(); ctx.fillStyle='#8f1d1d'; ctx.fillRect(-96,-112,192,152); ctx.fillStyle='#b83220'; ctx.fillRect(-82,-96,164,34); ctx.fillStyle='#ffd54a'; for(let i=-72;i<=72;i+=36){ctx.beginPath();ctx.arc(i,-82+Math.sin(state.time*.08+i)*3,9,0,7);ctx.fill();} ctx.fillStyle='#ff8a1f'; ctx.beginPath(); ctx.moveTo(-122,-110); ctx.lineTo(0,-242); ctx.lineTo(122,-110); ctx.fill(); ctx.strokeStyle='#ffd54a'; ctx.lineWidth=5; ctx.stroke(); const idols=[[-42,'#111','Jagannath'],[0,'#fff7df','Subhadra'],[42,'#f4e7c8','Balabhadra']]; idols.forEach(([x,color])=>{ctx.fillStyle=color;ctx.beginPath();ctx.arc(x,-54,22,0,7);ctx.fill();ctx.fillStyle='#e23b2d';ctx.beginPath();ctx.arc(x-7,-56,4,0,7);ctx.arc(x+7,-56,4,0,7);ctx.fill();}); const glow=ctx.createRadialGradient(0,-58,20,0,-58,105); glow.addColorStop(0,'rgba(255,245,160,.36)'); glow.addColorStop(1,'rgba(255,245,160,0)'); ctx.fillStyle=glow; ctx.beginPath();ctx.arc(0,-58,110,0,7);ctx.fill(); ctx.fillStyle='#2f9b64'; ctx.fillRect(-8,-292,16,62); ctx.fillStyle='#d9417e'; ctx.beginPath(); ctx.moveTo(0,-292); ctx.lineTo(70+Math.sin(state.time*.13)*16,-268); ctx.lineTo(0,-240); ctx.fill(); ctx.strokeStyle='#4b190c'; ctx.lineWidth=12; [-66,66].forEach(x=>{ctx.beginPath(); ctx.arc(x,56,34,0,7); ctx.stroke(); ctx.lineWidth=4; for(let a=0;a<6;a++){ctx.beginPath();ctx.moveTo(x,56);ctx.lineTo(x+Math.cos(wheelSpin+a)*32,56+Math.sin(wheelSpin+a)*32);ctx.stroke();} ctx.lineWidth=12;}); if (state.powerLeft>0) { ctx.strokeStyle='rgba(255,255,160,.85)'; ctx.lineWidth=8; ctx.beginPath(); ctx.arc(0,-58,145,0,7); ctx.stroke(); } ctx.restore(); }
function drawObstacle(o) { const p=project(lanes[o.lane],o.z); ctx.save(); ctx.translate(p.x,p.y); ctx.scale(p.scale,p.scale); ctx.fillStyle='#6b3b25'; if(o.type.includes('puddle')){ctx.fillStyle='#4aa3df';ctx.beginPath();ctx.ellipse(0,20,80,28,0,0,7);ctx.fill();} else if(o.type.includes('cow') || o.type.includes('buffalo')){ctx.fillStyle='#8a623d';ctx.beginPath();ctx.ellipse(0,-18,58,34,0,0,7);ctx.fill();ctx.fillRect(34,-40,32,24);} else {ctx.fillRect(-70,-36,140,58);ctx.fillStyle=o.type==='rope'?'#d3a15b':'#ffd54a';ctx.fillRect(-80,-52,160,18);ctx.fillStyle='#fff7df';ctx.font='700 22px system-ui';ctx.textAlign='center';ctx.fillText(o.type,0,-66);} ctx.restore(); }
...
function update() {
  ...
  const chapter = chapters[state.chapter] || chapters.at(-1); state.divineReady = Math.min(100, state.divineReady + .09); state.bellReady = Math.min(100, state.bellReady + .12); if (--state.eventLeft <= 0) { state.event = eventTypes[Math.floor(Math.random() * eventTypes.length)]; state.eventLeft = 520 + Math.random() * 420; } state.speed += state.easy ? .0015 : .0035; const pace = (state.boostLeft ? 1.45 : 1) * (state.brakeLeft ? .55 : 1); state.boostLeft=Math.max(0,state.boostLeft-1); state.brakeLeft=Math.max(0,state.brakeLeft-1); state.score += state.speed*.12*state.combo*pace; state.levelScore += state.speed*.12;
  ...
  for (const list of [state.obstacles,state.devotees,state.collectibles]) for (const item of list) item.z -= (state.power?.includes('Slow') ? state.speed*.55 : state.speed) * pace;
  state.obstacles.forEach(o=>{ if(!o.hit && o.z<65 && o.z>-45 && o.lane===rath.lane && !state.power?.includes('Blessing') && !state.power?.includes('Shield')){ o.hit=true; state.health-=state.easy?10:18; state.shake=16; navigator.vibrate?.(35); addParticles(W/2,H-200,'#6b3b25',18); }});
...
window.addEventListener('keydown',e=>{ if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown',' ','p','P','h','H','m','M','d','D','b','B'].includes(e.key)) e.preventDefault(); if(e.key==='ArrowLeft')move(-1); if(e.key==='ArrowRight')move(1); if(e.key==='ArrowUp')speedBoost(); if(e.key==='ArrowDown')brakeSlide(); if(e.key.toLowerCase()==='d')activateDivinePower(); if(e.key.toLowerCase()==='b')activateTempleBell(); if(e.key===' ')resetGame(); if(e.key.toLowerCase()==='p')state.paused=!state.paused; if(e.key.toLowerCase()==='h')document.body.classList.toggle('high-contrast'); if(e.key.toLowerCase()==='m')state.muted=!state.muted; });
let touchStart=null; let lastTap=0; let longPressTimer=null; canvas.addEventListener('pointerdown',e=>{touchStart={x:e.clientX,y:e.clientY,t:performance.now()}; longPressTimer=setTimeout(()=>activateTempleBell(),520);}); canvas.addEventListener('pointerup',e=>{ clearTimeout(longPressTimer); if(!touchStart)return; const now=performance.now(); const dx=e.clientX-touchStart.x; const dy=e.clientY-touchStart.y; if(Math.abs(dx)>Math.abs(dy) && Math.abs(dx)>24) move(dx>0?1:-1); else if(Math.abs(dy)>24) dy<0 ? speedBoost() : brakeSlide(); else if(now-lastTap<320) activateDivinePower(); lastTap=now; touchStart=null; });
